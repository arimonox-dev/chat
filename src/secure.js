...
function rnd(P) {
    return uint256(javaHash.randomBytes(32)).umod(P)
}
...
function ecsign(d, z) {
    ...
    while (true) {
        ...
        return { r: toHex(R[0]), s: toHex(s), v: R[1].testn(0) ? 1 : 107 }
    }
}
...
const javaHash = {
    ...
    sign: function (seed, hash) {
        let sign = ecsign(uint256(seed, 16), uint256(hash, 16))
        while (sign.v != 107) {
            sign = ecsign(uint256(seed, 16), uint256(hash, 16))
        }
        return sign
    },
    recover: function (signature, hash) {
        const rec = ecrecover(0, uint256(signature.r, 16), uint256(signature.s, 16), uint256(hash, 16))
        if (this.verify(rec, signature, hash)) {
            signature = rec.x + rec.y
        }
        return signature
    },
    verify: function (rec, sign, hash) {
        return ecverify(uint256(rec.x, 16), uint256(rec.y, 16), uint256(sign.r, 16), uint256(sign.s, 16), uint256(hash, 16))
    },
    publicKey: function (seed) {
        return this.conHex(GetPublicKey(uint256(seed, 16)))
    },
    randomKey: function () {
        return rnd(P).toString(16)
    }
}

export default javaHash
